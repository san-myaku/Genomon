...
function addPersonality(out: Candidate[], p: Phenotype): void {
  ...
  if (x.curiosity > 0.72) out.push({ kind: 'FIELD NOTE', text: '扉を開ける音より、閉める音に先に反応した。' });
  if (x.affection > 0.75) {
  ...
  if (x.tidiness > 0.76) out.push({ kind: 'FIELD NOTE', text: '寝床の位置を変えても、朝には同じ向きに整え直してある。' });
  if (x.energy < 0.23) out.push({ kind: 'OLD SAYING', text: '動かない個体を退屈だと言うのは、観察が短すぎる。' });
}
