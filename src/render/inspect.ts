...
/**
 * 別々の装飾の根元が離れていなければならない距離。
 *
 * 健全な個体では、植物（中央）と角・触角（`halfAt` の 50〜52%）は
 * 20 以上離れる。不良のときは 1 未満だった（実測 0.8）。
 * 間を広く取って 8 に置く。
 */
const ROOT_MIN_GAP = 8;
...
export function inspectModel(model: RenderModel): InspectResult {
  ...
  // ── 11. 頭頂の装飾が同じ場所から生えていないか ────────────
  //
  // 【なぜ要るか — 2026-08-16 に同じ形の指摘が 5 件出た】
  //   植物は「頭頂が混んでいたら横へ逃がす」処理を持っていたが、逃がす先
  //   （`halfAt` の 50%）が、角（52%）や触角（50%）と同じ帯だった。つまり
  //   **逃がした先が、逃げる相手そのもの**。実測でどれも 1 単位も離れて
  //   いなかったのに、既存の 10 項目は 1 件も拾えなかった（重なりを見て
  //   いたのは目どうしだけだったため）。
  //
  // 【矩形の重なりで見ない理由 — 実測して捨てた案】
  //   最初は装飾どうしの bbox の重なりで判定した。しかし角や触角は曲がった
  //   細い器官で外接矩形が空白だらけのため、隣り合っているだけの健全な個体
  //   まで重なりになり、成体 3000 体中 **998 件** の誤検出が出た。
  //   実際に不良を診断できた指標は矩形ではなく **根元の位置** だった。
  //   2 つの器官が同じ場所から生えているなら、必ずぶつかる。
  const rooted = model.parts.filter((p) => p.roots?.length);
  for (let i = 0; i < rooted.length; i++) {
    for (let j = i + 1; j < rooted.length; j++) {
      const a = rooted[i]!;
      const b = rooted[j]!;
      // 【横方向だけで測る理由 — 最初の版が本物のバグを捕まえられなかった】
      //   はじめは根元どうしのユークリッド距離で見ていた。ところが不良を
      //   出していた旧実装は、植物を横へ逃がすと同時に **下へも 0.14×H
      //   （約 15）ずらして** いた。そのため横が 0.8 しか離れていなくても
      //   距離は 15 を超え、検出 0 件になった（実際に旧実装へ戻して確認）。
      //   頭頂の装飾はどれも上へ伸びるので、**同じ列から生えているか**＝
      //   横方向の距離だけが効く。縦のずれは関係ない。
      let near = Infinity;
      for (const ra of a.roots!) {
        for (const rb of b.roots!) near = Math.min(near, Math.abs(ra.x - rb.x));
      }
      if (near < ROOT_MIN_GAP) {
        issues.push(`decor-same-root:${a.id}/${b.id}(${near.toFixed(1)})`);
      }
    }
  }
