...
function buildRarity(
  ...
  // 【左右差ではなく「揃っていること」を評価する（製品オーナー判断・2026-08-24）】
  //   以前は左右差が大きいほど加点していた。美しいのは揃っている側、という
  //   判断で反転した。
  //
  //   **単純に符号を反転してはいけない。** 左右差の分布は平均 0.27 で、
  //   0.40 以下が 83.9% を占める。「左右差が小さい＝加点」にすると
  //   ほぼ全個体に配ることになり、すぐ上のコメントにある
  //   「弱い個体に薄く配ると集団全体の底上げになってしまう」を自分で踏む。
  //
  //   **揃いは多面性ボーナスの「系統」には数えない**（下の families 参照）。
  //   ここが今回いちばん時間を使った所で、`tests/breeding.test.ts` の
  //   2 つの条件が正面からぶつかった:
  //
  //     ① 初期卵 500 体で precious がほぼ出ない（初期から量産されると
  //        交配の動機が消える）
  //     ② 12 世代の交配集団では precious が必ず 1 体以上出る
  //        （到達不能な tier は死んだ機能）
  //
  //   窓を広げると ① を割り、狭めると ② を割る。配点を 10→5 に下げても
  //   ① の件数はまったく変わらなかった ── 効いていたのは配点ではなく
  //   **系統を 1 本増やすこと**だった（3 系統目から `3*(n-2)^1.5` が跳ねる）。
  //   系統から外したうえで窓を 0.12 まで広げると、両方に収まる。
  //
  //     初期卵 500 体の precious   旧 3 体 → 4 体（上限 5 体）
  //     12 世代 3 系統の precious  旧 1/8/3 体 → 1/8/1 体（下限 1 体）
  //
  //   左右差は「高い側に広い領域」があったので浮動でも届いたが、揃いは
  //   値域の端（0）の狭い窓なので届きにくい。窓を動かすときは
  //   **必ず上の 2 つを両方**測り直すこと。片方だけ見ると必ずもう片方を割る。
  const symS = ramp(0.12 - num.asymmetry, 0, 0.12);
  const transS = ramp(num.translucency, 0.48, 0.78);
  const glowS = ramp(num.glow, 0.44, 0.74);
  add(10 * symS * symS, symS > 0.6 ? '左右のかたちが みごとに揃っている' : undefined);
  ...
  combo(eyeOdd && symS > 0.5, 10, 'ふつうとちがう目のならびが 左右きれいに揃っている');
  ...
  // 【揃い（symS）はここに数えない】
  //   数えると 3 系統目の跳ね（3*(n-2)^1.5）に乗って、初期卵から precious が
  //   量産される。揃いは「加点はするが、他の珍しさと相乗しない」扱い。
  const families =
