...
export const CAT_LOCI: readonly CatLocusDef[] = [
  ...
  {
    // 【新設 — 製品オーナーの指示（2026-08-22）】
    //   「目について、稀に（参考画像の）ようにキラキラがある個体がいる
    //     ようにしてほしい」。参考画像は、目の面に丸いつやが 2 つと
    //     4 方向に伸びる小さな星のきらめきが 1 つ入ったもの。
    //
    //   `pupil`（目の中の意匠）には足さない。意匠は 1 個体に 1 種類しか
    //   出せないので、そこへ入れると「きらめきが出た個体は、うずまきや
    //   はなびらを持てない」ことになり、どの意匠にも乗る飾りにできない。
    //   まつげ（`lashes`）を目の形と分けたのと同じ理由で独立させる。
    //
    // 【先頭が `none` であること — D-037】
    //   既存のセーブにはこの座が無く、読み込み側は **カタログ先頭の
    //   対立遺伝子をホモで補う**。先頭を `star` にすると、いま飼われて
    //   いる全個体の目に一斉にきらめきが生える。先頭は必ず `none`。
    //
    //   dominance を両方同じにして「発現率＝対立遺伝子頻度」にしてある
    //   （`lashes` と同じ設計）。weight 5 ＝ **20 体に 1 体**ほど。
    locus: 'eyeGlint',
    label: '目のきらめき',
    alleles: [
      { id: 'none', label: 'なし',   dominance: 3, weight: 95 },
      { id: 'star', label: 'きらり', dominance: 3, weight: 5, notable: true },
    ],
  },
  {
    // 【新設 — 製品オーナーの指示（2026-08-22）】
    //   「稀に目の中が二色の美しいグラデかかっている個体もいてほしい」。
    //
    //   `eyeGlint`（きらめき）と分ける。あちらは面の **上に載る白い飾り**、
    //   こちらは **面そのものの塗りかた**。同じ座にすると「きらめきと
    //   ふたいろを両方持つ個体」が作れなくなるうえ、片方を引くともう片方が
    //   出ないので、どちらも「稀」なのに互いの確率を食い合う。
    //
    // 【先頭が `none` であること — D-037】
    //   既存セーブにはこの座が無く、読み込み側はカタログ先頭の対立遺伝子を
    //   ホモで補う。先頭を `duo` にすると、いま飼われている全個体の目が
    //   一斉に二色になる。
    locus: 'irisTone',
    label: '目のグラデ',
    alleles: [
      { id: 'none', label: 'ふつう',   dominance: 3, weight: 94 },
      { id: 'duo',  label: 'ふたいろ', dominance: 3, weight: 6, notable: true },
    ],
  },
  {
    locus: 'lashes',
